...
const FILTER_OPTIONS = [
  ...
  { value: "WITHOUT_CONTACT", label: "Without Contact" },
  { value: "WITH_BUS", label: "With Bus Assigned" },
  { value: "WITHOUT_BUS", label: "Without Bus Assigned" }
];
...
function TodayAbsentStudents() {
  ...
  useEffect(() => {
    ...
    // Apply status filter
    if (filterStatus === "WITH_CONTACT") {
      result = result.filter(student => student.phone || student.email);
    } else if (filterStatus === "WITHOUT_CONTACT") {
      result = result.filter(student => !student.phone && !student.email);
    } else if (filterStatus === "WITH_BUS") {
      result = result.filter(student => student.busNumber);
    } else if (filterStatus === "WITHOUT_BUS") {
      result = result.filter(student => !student.busNumber);
    }
    ...
    result = [...result].sort((a, b) => {
      ...
      if (sortBy === "phone") {
        const aHasContact = a.phone || a.email ? 1 : 0;
        const bHasContact = b.phone || b.email ? 1 : 0;
        return bHasContact - aHasContact;
      }
      if (sortBy === "busNumber") {
        const aHasBus = a.busNumber ? 1 : 0;
        const bHasBus = b.busNumber ? 1 : 0;
        return bHasBus - aHasBus;
      }
      return 0;
    });
  ...
  const fetchTodayAbsentStudents = async () => {
    try {
      ...
      setFilteredStudents(absentStudents);
    } catch (err) {
      console.error(err);
      setError(
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(`Total Absent: ${filteredStudents.length} students`, 14, 38);
    
    const columns = ["Name", "Roll No", "Class", "Contact", "Status", "Last Updated"];
    ...
    doc.save(`absent_students_${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  const handleDownloadExcel = () => {
    const excelData = filteredStudents.map((s) => ({
      ...
      "School Name": JSON.parse(localStorage.getItem("user"))?.name || "-"
    }));
    ...
    XLSX.writeFile(workbook, `absent_students_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  const stats = {
    total: students.length,
    withContact: students.filter(s => s.phone).length,
    withoutContact: students.filter(s => !s.phone).length,
    withBus: students.filter(s => s.busNumber).length,
    withoutBus: students.filter(s => !s.busNumber).length
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-gray-50 to-white">
        ...
        <p className="text-gray-500 text-sm mt-2">Fetching today's absent students</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center p-4 bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl shadow-xl mb-6">
            <FaUserTimes className="text-white text-4xl" />
          </div>
          ...
          <div className="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-white border border-gray-200 rounded-xl shadow-sm">
            <FaCalendarDay className="text-amber-600" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">With Contact</p>
                <p className="text-3xl font-bold text-gray-900 mt-2">{stats.withContact}</p>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">Without Contact</p>
                <p className="text-3xl font-bold text-gray-900 mt-2">{stats.withoutContact}</p>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Absent Students Management</h3>
              <p className="text-gray-600 text-sm">Search, filter, and sort today's absent students</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="relative flex-1">
                <input
                  type="text"
                  placeholder="Search by name, roll no, class, email, or bus..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 pl-10 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all duration-200"
                />
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 pl-10 pr-8 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all duration-200 appearance-none bg-white"
                >
          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Showing {filteredStudents.length} of {students.length} absent students
        ...
        {filteredStudents.length > 0 && (
          <div className="flex flex-wrap gap-4 mb-8 p-6 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl">
            <div className="flex-1">
              <h3 className="font-semibold text-gray-900 mb-2 text-lg">Export Reports</h3>
              <p className="text-gray-600">Download absent student data for follow-up and records</p>
            </div>
          ...
          {filteredStudents.length === 0 ? (
              ...
              {students.length === 0 ? (
                <>
                  <div className="inline-flex items-center justify-center p-4 bg-green-100 rounded-full mb-4">
                    <FaUserTimes className="text-green-600 text-3xl" />
                  </div>
                  <h3 className="text-2xl font-semibold text-green-700 mb-2">
                    🎉 Perfect Attendance Today!
                  </h3>
                  <p className="text-gray-500 max-w-md mx-auto mb-6">
                    All students were successfully picked up. No absent students to display.
                  </p>
                ...
                <>
                  ...
                  <p className="text-gray-500 max-w-md mx-auto mb-6">
                    Try adjusting your search or filter criteria to find the student you're looking for.
                  </p>
                  {(searchTerm || filterStatus !== "ALL" || sortBy !== "name") && (
                    <button
                      onClick={() => {
            ...
            <>
              <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-amber-50 to-orange-50">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">Absent Students List</h3>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-amber-100 rounded-lg">
                              <FaUserTimes className="text-amber-600" />
                            </div>
                            <div>
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-900">{s.phone || "No phone"}</span>
                            </div>
                            {s.email && (
                              <div className="flex items-center gap-2">
                            ...
                            {!s.phone && !s.email && (
                              <span className="text-sm text-red-500 italic">No contact information</span>
                            )}
                          </div>
                        </td>
                        <td className="px-6 py-4">
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="text-sm text-gray-600">
                    <span className="font-medium">Summary:</span> {stats.total} absent students • {stats.withContact} have contact info • {stats.withoutContact} need contact details
                  </div>
                  <div className="text-sm text-amber-600 font-medium">
                    <FaExclamationTriangle className="inline mr-1" />
                    Action required: Contact parents of absent students
                  </div>
                </div>
              </div>
            </>
        ...
        {/* <div className="text-center text-gray-500 mt-12 pt-8 border-t border-gray-200">
          ...
          <p className="text-xs text-gray-400">
            Report generated on {new Date().toLocaleString()} • Data auto-refreshes every 10 minutes
          </p>
